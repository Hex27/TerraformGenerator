...
public class StrongholdPathPopulator extends PathPopulatorAbstract{
	...
	private void dropDownBlock(SimpleBlock block){
		if(block.getType().isSolid()){
			...
			if(GenUtils.chance(1, 3)){
				Material egg;
				try{
					egg  = Material.valueOf("INFESTED_"+type.toString());
	...
	private boolean setIronBars(PathPopulatorData ppd){
		...
		for(SimpleBlock b:toBeGate) b.setType(Material.IRON_BARS);
		for(SimpleBlock b:toBeGate) BlockUtils.correctMultifacingData(b);
		return true;
	}
}
