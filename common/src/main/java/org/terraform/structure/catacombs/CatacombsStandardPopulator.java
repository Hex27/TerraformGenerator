...
//            	//Flooring
            	...
            	if(ceiling != null) {
            		...
            		int extend = Math.round(((xDiffFromCent+zDiffFromCent)/(maxTotalDiff))
            				*maxDownExtend);
    	...
    	for(int i = 0; i < 1+rand.nextInt(3); i++)
    	{
    		...
    		if(data instanceof PopulatorDataPostGen) {
    			Skeleton e = (Skeleton) ((PopulatorDataPostGen) data).getChunk().getWorld()
    					.spawnEntity(new Location(((PopulatorDataPostGen) data).getChunk().getWorld(),target.getX()+0.5,target.getY()+0.3,target.getZ()+0.5),
    							EntityType.SKELETON);
