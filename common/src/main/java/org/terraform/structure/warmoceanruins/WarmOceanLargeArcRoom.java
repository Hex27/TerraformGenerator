...
public class WarmOceanLargeArcRoom extends WarmOceanBaseRoom {
    ...
    public void populate(PopulatorDataAbstract data, CubeRoom room) {
        ...
        Wall chestTarget = new Wall(room.getCenterSimpleBlock(data), facing)
                .getLeft(GenUtils.getSign(rand)*rand.nextInt(2))
                .getFront(GenUtils.getSign(rand)*rand.nextInt(6))
                .getGround().getUp();
        new ChestBuilder(Material.CHEST)
                .setFacing(BlockUtils.getDirectBlockFace(rand))
                .setLootTable(TerraLootTable.UNDERWATER_RUIN_BIG)
                .setWaterlogged(chestTarget.getY() <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
                .apply(chestTarget);
        ...
        for(int i = 0; i < 3; i++)
        {
            ...
            if(rand.nextBoolean()) ((Slab) centreSlabs[i]).setType(Slab.Type.TOP);
            if(GenUtils.chance(rand,1,4)) centreSlabs[i] = Bukkit.createBlockData(Material.CHISELED_SANDSTONE);
            sideSlabs[i] = Bukkit.createBlockData(Material.CUT_SANDSTONE_SLAB);
            if(rand.nextBoolean()) ((Slab) sideSlabs[i]).setType(Slab.Type.TOP);
        }
        ...
        for(BlockFace pillarDir:BlockUtils.getAdjacentFaces(facing)) {
            ...
            {
                new StairBuilder(Material.SANDSTONE_STAIRS)
                        .setHalf(Bisected.Half.TOP)
                        .setFacing(pillarDir)
                        .setWaterlogged(pillarCore.getY()+6 <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
                        .apply(pillarCore.getUp(6)
                                .getRelative(face,2)
                                .getRelative(pillarDir.getOppositeFace(), 3))
                        .setWaterlogged(pillarCore.getY()+7 <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
                        .apply(pillarCore.getUp(7)
                                .getRelative(face,2)
                                .getRelative(pillarDir.getOppositeFace(), 4));
            ...
            for(BlockFace face:BlockUtils.directBlockFaces)
            {
                for(BlockFace dir:BlockUtils.getAdjacentFaces(face)) {
                    pillarCore.getRelative(face, 3).getRelative(dir)
                            .setType(Material.CUT_SANDSTONE_SLAB);

                    new StairBuilder(Material.SANDSTONE_STAIRS)
                            .setFacing(dir)
                            .setWaterlogged(pillarCore.getY() <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
                            .apply(pillarCore.getRelative(face, 3).getRelative(dir, 2));
                }
                new StairBuilder(Material.SANDSTONE_STAIRS)
            ...
            for(int fb = -4; fb <= 4; fb++) {
                if(Math.abs(lr) < 9 && Math.abs(fb) < 4) {
                    ...
                    if(Math.abs(lr) < 7 && Math.abs(fb) < 2)
                        ...
                        new SlabBuilder(Material.CUT_SANDSTONE_SLAB)
                                .setType(Slab.Type.TOP)
                                .setWaterlogged(core.getY()+4 <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
                                .apply(core.getLeft(lr).getFront(fb).getUp(4));
                }else
                    new SlabBuilder(Material.CUT_SANDSTONE_SLAB)
                            .setType(Slab.Type.TOP)
                            .setWaterlogged(core.getY() <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
            ...
            for(int i = 0; i < entry.getValue(); i++)
            {
                ...
                if(i == 0)
                {
                    ...
                    new StairBuilder(Material.SANDSTONE_STAIRS).setFacing(BlockUtils.getRight(w.getDirection()))
                            .apply(w.getLeft())
                            .setHalf(Bisected.Half.TOP)
                            .setWaterlogged(w.getY()+2 <= TConfigOption.HEIGHT_MAP_SEA_LEVEL.getInt())
