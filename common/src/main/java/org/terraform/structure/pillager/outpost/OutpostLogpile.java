    			...
    			for(int len = 0; len <= logLength; len++) {
    				...
    				if(len == 1) {
    					...
    					if(i == 0) {
    						...
    						.apply(core.getRelative(0,i+1,0).getLeft(width+1).getFront(len))
    						.apply(core.getRelative(0,i+1,0).getLeft(width+1).getRear(len))
    						.setFacing(BlockUtils.getLeft(core.getDirection()))
    						.apply(core.getRelative(0,i+1,0).getRight(width+1).getFront(len))
