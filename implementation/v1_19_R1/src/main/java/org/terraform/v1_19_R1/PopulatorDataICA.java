...
public class PopulatorDataICA extends PopulatorDataICABiomeWriterAbstract {
	...
	public void setBiome(int rawX, int rawY, int rawZ, CustomBiomeType cbt, Biome fallback) {
		IRegistry<BiomeBase> biomeRegistry = MinecraftServer.getServer().aX().b(IRegistry.aR);
		...
		if(cbt == CustomBiomeType.NONE) {
			...
			Optional<Holder<BiomeBase>> optHolder = biomeRegistry.b(rkey); //getHolder
...
//        }
    ...
    public void registerNaturalSpawns(NaturalSpawnType type, int x0, int y0, int z0, int x1, int y1, int z1) {
    	ResourceKey<Structure> structureKey = BuiltinStructures.l; //Monument
    	switch(type) {
    	case GUARDIAN:
    	...
    	//aN is STRUCTURE_REGISTRY
    	IRegistry<Structure> featureRegistry = MinecraftServer.getServer().aX().b(IRegistry.aN);
