			...
			for(int i = 0; i < 6; i++) {
				...
				if(i==0) {
					...
					if(BlockUtils.isAir(roofPiece.getDown().getType())
						...
						.setFacing(side)
						.setHalf(Half.TOP)
						.apply(roofPiece.getDown());
					
					if(i == 5 
							&& (roofPiece.getDown().getRelative(side).getType() == Material.COBBLESTONE_STAIRS
							|| roofPiece.getDown().getRelative(side.getOppositeFace()).getType() == Material.COBBLESTONE_STAIRS))
					{
