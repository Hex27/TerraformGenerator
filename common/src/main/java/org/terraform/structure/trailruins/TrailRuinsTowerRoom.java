    ...
    public static class TrailRuinsTowerParser extends SchematicParser{
        ...
        public void applyData(@NotNull SimpleBlock block, @NotNull BlockData data) {
            ...
            if(newMapping.containsKey(data.getMaterial())) {
                ...
                else data = Bukkit.createBlockData(newMapping.get(data.getMaterial()));
            ...
            for(int i = 1; i < 3; i++)
            {
                if(GenUtils.chance(random, 1, 4) && BlockUtils.isStoneLike(block.getUp(i).getType()))
                {
                    ...
                    {
                        block.getUp(i).setType(OneTwentyBlockHandler.SUSPICIOUS_GRAVEL);
                        block.getPopData().lootTableChest(block.getX(), block.getY()+i, block.getZ(),
                                GenUtils.chance(random,1,3)? TerraLootTable.TRAIL_RUINS_ARCHAEOLOGY_RARE: TerraLootTable.TRAIL_RUINS_ARCHAEOLOGY_COMMON);
                    }
                    else
