...
public class RuinedPortalPopulator extends MultiMegaChunkStructurePopulator {
    ...
    public void spawnRuinedPortal(TerraformWorld tw, @NotNull Random random, @NotNull SimpleBlock core,
    	...
    	new CylinderBuilder(random, w.getDown(), stoneBricks)
    	.setRadius(horRadius+1)
    	.setRY(1)
    	...
    	if(effectiveGround != null) {
	    	...
	    	new StalactiteBuilder(portalBlocks)
	    	.setVerticalSpace(Math.round(2.5f*heightCorrection*1.5f))
	    	.setFacingUp(true)
    	...
    	for(int left = 0; left < horRadius; left++) {
    		...
    		w.getUp(vertHeight+1).getLeft(left).setType(Material.STONE_BRICK_SLAB, getFluid(w.getUp(vertHeight+1).getLeft(left)));
    	}
    	for(int right = 1; right < horRadius-1; right++) {
    		...
    		w.getUp(vertHeight+1).getRight(right).setType(Material.STONE_BRICK_SLAB, getFluid(w.getUp(vertHeight+1).getRight(right)));
    	}
    	 ...
    	 for(int relx = - horRadius*3; relx < horRadius*3; relx++) {
    		 for(int relz = - horRadius*3; relz < horRadius*3; relz++) {
    	         ...
    	         if(fissureNoise > 0) {
    	        	 ...
    	        	 if(lowestY > target.getY()) lowestY = target.getY();
    	         }
        	 }
    	 }
