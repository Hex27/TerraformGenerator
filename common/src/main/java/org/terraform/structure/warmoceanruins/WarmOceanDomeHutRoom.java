...
public class WarmOceanDomeHutRoom extends WarmOceanBaseRoom{
    ...
    public void populate(PopulatorDataAbstract data, CubeRoom room) {
        ...
        new CylinderBuilder(rand, centre.getDown(), Material.SMOOTH_SANDSTONE, Material.SANDSTONE)
                ...
                .setHardReplace(true)
                .build();
        new CylinderBuilder(rand, centre, Material.WATER)
                .setStartFromZero(true)
                .setRadius(radius-1)
                .setRY(cylSize)
                .setNoiseMagnitude(0)
                .setHardReplace(true)
                .build();
        new SphereBuilder(rand, centre.getUp(cylSize-1), Material.SMOOTH_SANDSTONE, Material.SANDSTONE)
                .setRadius(radius)
                .setSphereType(SphereBuilder.SphereType.UPPER_SEMISPHERE)
                .setSmooth(true)
                .build();
        new SphereBuilder(rand, centre.getUp(cylSize-1), Material.WATER)
                .setRadius(radius-1)
                .setSphereType(SphereBuilder.SphereType.UPPER_SEMISPHERE)
                .setSmooth(true)
                .setHardReplace(true)
        ...
        for(int i = 0; i < GenUtils.randInt(rand,2,4); i++)
        {
            BlockUtils.replaceWaterSphere(i*room.getX()*room.getZ(),
                    GenUtils.randInt(2, 4),
                    centre.getRelative(GenUtils.getSign(rand)*rand.nextInt((int)radius),
