...
public class SulfurNoiseClusterPopulator extends AbstractNoiseClusterPopulator{
    ...
    public boolean canSpawnCluster(TerraformWorld tw, int x, int y, int z) {
        ...
        return thresholdNoise.GetNoise(x,y,z) > TConfig.c.BIOME_CAVE_SULFUR_THRESHOLD;
    }
    @Override
                        ...
                        @NotNull SimpleBlock floor)
    {
        ...
        if (GenUtils.chance(tw.getHashedRand(ceil.getX(),ceil.getY(),ceil.getZ()),
                (int) (TConfig.c.BIOME_CAVE_SULFUR_SPRING_CHANCE * 1000), 1000)){
        ...
        BlockUtils.lambdaCircularPatch(lambdaSeed,
                ...
                core, (cons)->{
                    ...
                    if(dsqr <= 9) //Clear the space above the pool and dig down
                    {
                        ...
                        cons.getAtY(waterY).Pillar(1+waterDepth, Material.WATER);
                        if(GenUtils.chance(1,5))
