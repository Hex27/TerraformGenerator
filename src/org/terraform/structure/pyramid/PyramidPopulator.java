...
public class PyramidPopulator extends SingleMegaChunkStructurePopulator {
    ...
    private static boolean rollSpawnRatio(TerraformWorld tw, int chunkX, int chunkZ) {
...
//		private int roomMinZ = 10;
        ...
        level1.registerRoomPopulator(new HuskTombPopulator(random, false, false));
        ...
        //Level 3
        RoomLayoutGenerator level3 = new RoomLayoutGenerator(hashedRand, RoomLayout.RANDOM_BRUTEFORCE, numRooms, x, y + 15, z, range);
        level3.setRoomMaxX(7);
        level3.setRoomMinX(5);
        level3.setRoomMaxZ(6);
        level3.setRoomMinZ(5);
        level3.setRoomMaxHeight(5);
        level3.setPathPopulator(new PyramidPathPopulator(tw.getHashedRand(x, y + 15, z, 2233), 2));

        //Stairway (Level 2 to 3)
        CubeRoom stairway = level2.forceAddRoom(5, 5, 9);
        stairway.setRoomPopulator(new PyramidStairwayRoomPopulator(random, false, false));
        CubeRoom stairwayTop = new CubeRoom(5, 5, 5, stairway.getX(), y + 15, stairway.getZ());
        stairwayTop.setRoomPopulator(new PyramidStairwayTopPopulator(random, false, false));
        level3.getRooms().add(stairwayTop);
        ...
        level3.generate(false);
        level3.fill(data, tw, Material.SANDSTONE, Material.CUT_SANDSTONE);
    }
